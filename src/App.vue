...
//watch anlık değişen değeri izleyip gormemizi saglar
watch(name,(newVal)=>{
  localStorage.setItem('name',newVal)
})
// refresh edince degisen degeri local storage koydugumuz değeri gormesi kaydetmesi icin onmounted fonksiyonu 
onMounted(()=>{
...
<template>
  <main class="app">
    ...
    <section class="todo-item">
        <h3>TODO LIST</h3>
          <div class="list" id="todo-item">
            <div v-for= "todo in todos_asc" :key="`todo-item ${todo.done &&'done'}`">
              <label>
                  <input type="checkbox" v-model="todo.done"/>
                  <span :class="`bubble ${todo.category=='business' ? 'business' 
								: 'personal'}`"></span>
              </label>
